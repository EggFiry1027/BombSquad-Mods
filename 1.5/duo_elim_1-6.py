...
# ba_meta export game
class DuoEliminationGame(ba.TeamGameActivity[Player, Team]):
    """Game type where team with player(s) left alive win."""

    name = 'Duo Elimination'
    description = 'Team with player(s) remaining alive wins.'
    ...
    def get_available_settings(
        ...
        if issubclass(sessiontype, ba.DualTeamSession):
            settings.append(ba.BoolSetting('Duo Mode', default=True))
            settings.append(
    ...
    def __init__(self, settings: dict):
        ...
        self._duo_mode = bool(settings.get('Duo Mode', True))
    ...
    def _update_duo_mode(self) -> None:
                ...
                for team in self.teams:
                    ...
                    while True:
                        ...
                        for player in players_with_lives:
                            player.icons.append(
                                Icon(player,
                                     position=(xval, (36 if duo_limit > 0 else 25)),
                                     scale=0.9 if duo_limit > 0 else 0.5,
                                     name_maxwidth=85 if duo_limit > 0 else 75,
                                     name_scale=0.8 if duo_limit > 0 else 1.0,
                                     flatness=0.0 if duo_limit > 0 else 1.0,
                                     shadow=0.5 if duo_limit > 0 else 1.0,
                                     show_death=True if duo_limit > 0 else False,
                                     show_lives=False))
                            xval += x_offs * (0.85 if duo_limit > 0 else 0.56)
                            duo_limit -= 1
                        test_lives += 1
            # Non-solo mode.
            else:
